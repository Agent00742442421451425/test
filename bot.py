...
async def sync_stock_handler(query):
    ...
    try:
        ...
        if not stock_for_api:
            # Нет остатков с привязкой к товару — предлагаем добавить аккаунты с товаром или править accounts.json
            ...
            if no_sku_count:
                msg = (
                    ...
                    "• Чтобы остатки уходили в Маркет — добавляйте аккаунты *с выбором товара* (кнопка ниже) или пропишите поле `sku` в `accounts.json` у существующих записей."
                )
                reply_markup = InlineKeyboardMarkup([
                    [_btn("➕ Добавить аккаунты с привязкой к товару", "add_accounts", style="success")],
                    [_btn("📦 Склад", "stock_info")],
                    [_btn("⬅️ В меню", "back_menu")],
                ])
            else:
                msg = (
                    "⚠️ *Нет товаров для синхронизации*\n\n"
                    "На складе нет свободных аккаунтов. Добавьте аккаунты с привязкой к товару."
                )
                reply_markup = InlineKeyboardMarkup([
                    [_btn("➕ Добавить аккаунты", "add_accounts", style="success")],
                    [_btn("⬅️ В меню", "back_menu")],
